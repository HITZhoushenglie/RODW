...
from rein_learning.blocks.SENet import SENet, SENet_decoder
...
# def __init__(self, image_size=64, message_length=16, in_channels=4, channels=64, norm_type="batch", final_skip=False, diffusion_length=256):

class EncoderUnet(nn.Module):
    def __init__(self, image_size=64, message_length=16, in_channels=4, channels=64, norm_type="batch", final_skip=False, msg_feat_size = 12, diffusion_length=256):
        super(EncoderUnet, self).__init__()


        # revise 1
        message_convT_blocks = int(np.log2(image_size // int(np.sqrt(diffusion_length))))

        # revise 2
        # self.fc = nn.Linear(message_length, msg_feat_size * msg_feat_size)
        # self.msg_feat_size = msg_feat_size
        # message_convT_blocks = int(np.log2(image_size // msg_feat_size))
        ...
        # revise 1
        l = int(np.sqrt(message_length))
        self.up_factor = 16 / l

        # revise 3
        # l = int(np.sqrt(message_length))
        # self.up_factor = msg_feat_size / l
    ...
    def forward(self, image, message):
        ...
        message_image = message.view(-1, 1, size, size) # [2 1 6 6]
        expanded_message = F.interpolate(message_image, scale_factor=self.up_factor, mode='nearest')

        # size = int(np.sqrt(message.shape[1]))
        #  message_image = message.view(-1, 1, size, size)
        message_pre = self.message_pre_layer(expanded_message)
        # print("encoder")
        # print(message_pre.shape)

        # concatenate
        # print(message_pre.shape, image.shape) # [2 64 192 192] [2 3 256 256]
...
"""
    def forward(self, image, message):
        # Message Processor # revise 2
        # msg_feat = self.fc(message)  # [B, 16*16*channels]
        # msg_feat = msg_feat.view(-1, 1, self.msg_feat_size, self.msg_feat_size)  # [B, channels, 16, 16]
        # message_pre = self.message_pre_layer(msg_feat)

        # Message Processor # revise 3
        # 128 -> 256
        # x_padded = F.pad(message, (0, 128)) 
        # x_padded = message.repeat(1, 2) 
        # x_padded = x_padded.view(-1, 1, self.msg_feat_size, self.msg_feat_size)
        # message_pre = self.message_pre_layer(x_padded)

        # 100 -> 144
        size = int(np.sqrt(message.shape[1]))
        message = message.view(-1, 1, size, size)
        x_padded = F.interpolate(message, scale_factor=self.up_factor, mode='nearest')
        message_pre = self.message_pre_layer(x_padded)
...
class DecoderResnet(nn.Module):
    ...
    def __init__(self, image_size=64, message_length=16, in_channels=3, norm_type="batch", diffusion_length=256, msg_feat_size=12):
        super(DecoderResnet, self).__init__()

        # revise 1 
        stride_blocks = int(np.log2(image_size // int(np.sqrt(diffusion_length))))

        # revise 2/3
        # stride_blocks = int(np.log2(image_size // msg_feat_size))
        ...
        # revise 1
        l = int(np.sqrt(message_length))
        self.down_factor = l / 16

        # revise 2
        # self.fc = nn.Linear(msg_feat_size * msg_feat_size, message_length) 

        # revise 3
        # l = int(np.sqrt(message_length))
        # self.down_factor = l / msg_feat_size
    ...
    def forward(self, x):
        ...
        # revise 1
        y = F.interpolate(y, scale_factor=self.down_factor, mode='nearest')
